...
from app.bot.database.db_funcs import get_catalog, get_subcategories, get_products, get_current_product, \
    get_all_products, \
    update_order, add_to_users, get_FAQ
...
async def show_answer(callback: CallbackQuery,
                      ...
                      text: str):
    session = dialog_manager.middleware_data['session']
    FAQ_list = await get_FAQ(session)
...
async def getter_question(dialog_manager: DialogManager,
                          ...
                          **kwargs):
    session = dialog_manager.middleware_data['session']
    FAQ = await get_FAQ(session)
    question = list(FAQ)[dialog_manager.dialog_data['question_num']]
...
async def getter_FAQ(dialog_manager: DialogManager,
    ...
    session = dialog_manager.middleware_data['session']
    FAQ = await get_FAQ(session)
    FAQ_list = [item[0] for item in FAQ]
    logging.info(FAQ_list)
    return {"questions": FAQ_list}
...
@router.inline_query()
async def faq_answers(query: InlineQuery, **kwargs):
    session = kwargs['session']
    FAQ = await get_FAQ(session)
