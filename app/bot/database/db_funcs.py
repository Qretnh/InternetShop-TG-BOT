...
from .db import Catalog, SubCategory, Product, Order, User, FAQ
...
async def get_FAQ(session: AsyncSession):
    stmt = select(FAQ)
    result = await session.execute(stmt)
    result = [[item.question, item.answer] for item in result.scalars().all()]
    return result